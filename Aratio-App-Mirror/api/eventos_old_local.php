        ...
        case 'POST':
            ...
            $stmt = $db->prepare("
                INSERT INTO eventos (
                    ...
                    asistentes_esperados, presupuesto, estado, notas
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");

            $success = $stmt->execute([
                $data['campana_id'],
                $user['id'], // El usuario actual es el responsable por defecto
                $data['nombre'],
                $data['tipo'],
                $data['descripcion'] ?? null,
                str_replace('T', ' ', $data['fecha_inicio']),
                !empty($data['fecha_fin']) ? str_replace('T', ' ', $data['fecha_fin']) : null,
                $data['ubicacion'],
                $data['direccion'] ?? null,
                ...
                $data['barrio'] ?? null,
                !empty($data['asistentes_esperados']) ? $data['asistentes_esperados'] : 0,
                !empty($data['presupuesto']) ? $data['presupuesto'] : 0,
                $data['estado'],
                $data['notas'] ?? null
            ]);
        ...
        case 'PUT':
            ...
            $stmt = $db->prepare("
                UPDATE eventos SET
                    ...
                    asistentes_esperados = ?, presupuesto = ?,
                    estado = ?, notas = ?
                WHERE id = ?
            ");
