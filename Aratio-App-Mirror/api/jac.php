<?php
/**
 * API REST - Módulo JAC (Juntas de Acción Comunal)
...
if ($method === 'GET') {
    ...
    // ── Planchas de una JAC (incluye bloques v1.7.0) ─────────────────────────
    if ($action === 'planchas') {
...
if ($method === 'POST') {
    ...
    $stmt->execute([
        ...
        san($data['territorio_valle_nombre']), // legacy field
        san($data['sector'] ?? ''),
        san($data['comuna'] ?? ''),
    ...
    echo json_encode(['success'=>true,'message'=>'JAC creada','id'=>$db->lastInsertId()]);
    exit;
}
...
if ($method === 'PUT') {
    ...
    echo json_encode(['success'=>true,'message'=>'JAC actualizada']);
    exit;
}
