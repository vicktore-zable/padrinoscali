...
function handleGet($db, $userId)
{
    ...
    if ($colaboradorId) {
        ...
        if ($search) {
            $sql .= " AND (c.nombres LIKE ? OR c.apellidos LIKE ? OR c.documento LIKE ?)";
            $searchTerm = "%$search%";
            $params[] = $searchTerm;
            $params[] = $searchTerm;
            $params[] = $searchTerm;
...
function handleGetLideres($db)
{
    ...
    foreach ($lideres as &$l) {
        $l['nombre_completo'] = $l['nombres'] . ' ' . $l['apellidos'];
    }
...
function handlePost($db, $userId)
{
    ...
    $stmt = $db->prepare("
        INSERT INTO colaboradores (
            ...
            tipo_territorio, territorio, barrio, direccion, detalle_ubicacion,
...
function procesarArchivoImport($db, $filePath, $campanaId, $userId, $liderDocumento, $ext)
{
    ...
    // Por simplicidad, soportamos CSV
    ...
    if ($ext === 'csv') {
        ...
        while (($row = fgetcsv($handle, 0, ',')) !== false) {
            ...
            try {
                ...
                $data = [
                    ...
                    'lider_documento' => $row[$map['lider_documento'] ?? -1] ?? null,
                    'observaciones' => $row[$map['observaciones'] ?? -1] ?? null,
                ];

                // Validar campos requeridos
                $required = ['nombres', 'apellidos', 'documento', 'fecha_nacimiento', 'genero', 'perfil', 'departamento', 'municipio'];
                foreach ($required as $field) {
                    if (empty($data[$field])) {
                ...
                $stmt = $db->prepare("SELECT id FROM colaboradores WHERE documento = ? AND campana_id = ?");
                $stmt->execute([$data['documento'], $campanaId]);
                if ($stmt->fetch()) {
                    throw new Exception("Documento duplicado: {$data['documento']}");
                }
                ...
                $stmt->execute([
                    ...
                    !empty($data['lider_documento']) ? sanitize($data['lider_documento']) : $liderDocumento, // Prioridad: CSV > Global
                    sanitize($data['puesto_votacion']),
                    sanitize($data['mesa_votacion']),
                    sanitize($data['observaciones']),
                    $userId
                ]);
        ...
        // Para xlsx/xls se necesita librería adicional
        $resultado['errores'][] = [
            'fila' => 0,
            'error' => 'Para archivos Excel (.xlsx/.xls) use formato CSV. Próximamente soporte Excel.'
        ];
    }
        ...
        if ($exists) {
            ...
            $stmt = $db->prepare("
                INSERT INTO curriculum (
                    colaborador_id, experiencia_laboral, formacion_academica, participacion_politica,
                    resumen_profesional, habilidades, idiomas, reconocimientos, referencias, observaciones
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $colaboradorId,
...
function handleExport($db)
{
    ...
    if ($formato === 'csv') {
        ...
        header('Content-Disposition: attachment; filename="colaboradores_export_' . date('Y-m-d') . '.csv"');
        ...
        jsonResponse([
            'success' => false,
            'message' => 'Formato Excel no disponible. Use CSV.'
        ], 400);
    }
}

/**
 * GET: Descargar plantilla de importación
 */
function handlePlantilla()
{
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="plantilla_colaboradores.csv"');
    ...
    fputcsv($output, [
        ...
        'Comuna 5',
        'Barrio El Poblado',
        'Líder de junta de acción comunal'
    ]);
...
/**
 * GET: Estadísticas extendidas usando la vista
 */
function handleStatsExtended($db)
{
    $campanaId = $_GET['campana_id'] ?? null;
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT * FROM v_estadisticas_campana WHERE campana_id = ?");
        $stmt->execute([$campanaId]);
        $stats = $stmt->fetch();
        ...
        if ($stats) {
            ...
            jsonResponse([
                ...
                'data' => [
                    ...
                    'total_lideres_unicos' => 0,
                    'sin_lider' => 0
                ]
            ]);
        }
    } catch (Exception $e) {
        // Vista no existe, usar query manual
        handleStats($db);
    }
}

/**
 * POST: Actualización rápida (Nivel, Perfil, Estado) desde modal
 */
function handleQuickUpdate($db, $userId)
{
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        jsonResponse(['success' => false, 'message' => 'Método debe ser POST'], 405);
    }
    ...
    $nivel = $data['nivel_participacion'] ?? null;
    
    if (!$id || !$perfil || !$estado || !$nivel) {
        jsonResponse(['success' => false, 'message' => 'Faltan datos requeridos (id, perfil, estado, nivel)'], 400);
    }
    
    // Convertir estado virtual a números
    $potencial = 0;
    $historico = 0;
    switch ($estado) {
        case 'Nuevo': $potencial = 0; $historico = 0; break;
        case 'Desvinculado': $potencial = 1; $historico = 0; break;
        case 'Creció': $potencial = 1; $historico = 2; break;
        case 'Decrece': $potencial = 2; $historico = 1; break;
        case 'Igual': $potencial = 1; $historico = 1; break;
    }
    ...
    try {
        ...
        jsonResponse(['success' => true, 'message' => 'Colaborador actualizado rápidamente']);
    } catch (Exception $e) {
