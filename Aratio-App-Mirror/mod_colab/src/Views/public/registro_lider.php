<?php
/**
 * PÁGINA PÚBLICA: Registro de Líderes - Edición Aratio Gold v2.6.8
 * Diseño Premium Magenta/Gold + Perfil Profesional + Jerarquía Geográfica 4 Niveles
 */
?>
<!DOCTYPE html>
<html lang="es">
...
<body class="bg-slate-50 min-h-screen pb-10" x-data="registrationForm()" x-init="init()">
                ...
                <form @submit.prevent="submitForm" class="space-y-10">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                <label class="label-gold">Campaña de Destino *</label>
                            ...
                            <div x-show="form.campana_id" x-cloak class="pt-6 border-t border-slate-100 transition-all">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" 
                                           ...
                                           placeholder="Escribe nombre de tu referente..." 
                                ...
                                <div x-show="form.lider_directo" 
                                    ...
                                    <div class="flex-1">
                                        <p class="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-0.5 tracking-tighter">Líder que te refiere</p>
                    ...
                    <div x-show="form.campana_id" x-cloak class="space-y-6 pt-6 border-t border-slate-100">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="text-2xl font-black text-slate-800 tracking-tight">Datos del Líder</h2>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div><label class="label-gold">WhatsApp *</label><input type="tel" x-model="form.telefono" required class="input-gold" placeholder="3XX XXX XXXX"></div>
                        </div>
                    </div>
                    ...
                    <div x-show="form.campana_id" x-cloak class="bg-slate-50/70 p-8 rounded-[2rem] border border-slate-100 space-y-8">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="text-2xl font-black text-slate-800 tracking-tight">Ubicación Territorial</h2>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-full grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="label-gold text-slate-400">Zona</label>
                                ...
                                <div>
                                    <label class="label-gold text-slate-400">Sector</label>
                                ...
                                <div class="col-span-full">
                                    <label class="label-gold font-black text-primary uppercase text-[10px] tracking-widest">Barrio / Vereda de Acción</label>
                                    <select x-model="form.barrio" :disabled="!form.territorio" class="input-gold border-primary/20 font-bold text-primary disabled:opacity-50">
                                        <option value="">-- Confirma el Barrio --</option>
                        ...
                        <div class="pt-6 border-t border-slate-200">
                            <label class="label-gold text-slate-800 font-black mb-4 flex items-center gap-2">
                                <i data-lucide="vote" class="w-4 h-4"></i> Información Electoral
                            </label>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div class="md:col-span-2">
                                    <select x-model="form.puesto_votacion" required :disabled="!form.cod_mpio" class="input-gold bg-white border-primary/10">
                                ...
                                <div>
                                    <input type="text" x-model="form.mesa_votacion" placeholder="Mesa" class="input-gold text-center">
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- SECCIÓN 4: PERFIL PROFESIONAL (CV) -->
                    <div x-show="form.campana_id" x-cloak class="space-y-6 pt-6 border-t border-slate-100">
                        <div class="flex items-center gap-3">
                            <span class="w-10 h-10 rounded-full bg-magenta-50 flex items-center justify-center text-primary shadow-inner">
                                <i data-lucide="briefcase" class="w-5 h-5"></i>
                            </span>
                            <h2 class="text-2xl font-black text-slate-800 tracking-tight">Perfil Profesional</h2>
                        </div>
                        
                        <div class="space-y-6">
                            <div>
                                <label class="label-gold">Resumen Profesional</label>
                                <textarea x-model="form.resumen_profesional" rows="3" class="input-gold resize-none" placeholder="Describe brevemente tu experiencia laboral y política..."></textarea>
                            </div>

                            <!-- Áreas de Especialización -->
                            <div>
                                <label class="label-gold">Áreas de Especialización / Interés</label>
                                <div class="flex flex-wrap gap-2">
                                    <template x-for="area in allAreas" :key="area">
                                        <button type="button" 
                                                @click="toggleInterest(area)" 
                                                class="px-4 py-2 rounded-xl text-xs font-bold border-2 transition-all shadow-sm"
                                                :class="form.areas_interes.includes(area) ? 'bg-primary border-primary text-white' : 'bg-white border-slate-100 text-slate-400 hover:text-primary'">
                            ...
                            <div class="space-y-4">
                                <div class="flex justify-between items-center bg-slate-50 p-3 rounded-xl">
                                    <span class="text-xs font-black uppercase text-slate-400 tracking-widest">Añadir Formación Académica</span>
                                    <button type="button" @click="addFormacion" class="text-primary font-bold text-xs hover:underline">+ Agregar</button>
                                </div>
                                <template x-for="(f, i) in form.formaciones" :key="i">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-5 bg-white border border-slate-100 rounded-2xl shadow-sm relative animate-fade-up">
                                        <button type="button" @click="removeFormacion(i)" class="absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center"><i data-lucide="x" class="w-3 h-3"></i></button>
                                        <input type="text" x-model="f.titulo" placeholder="Título / Taller" class="input-gold py-2 text-sm">
                                        <input type="text" x-model="f.institucion" placeholder="Institución" class="input-gold py-2 text-sm">
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex justify-between items-center bg-slate-50 p-3 rounded-xl">
                                    <span class="text-xs font-black uppercase text-slate-400 tracking-widest">Añadir Experiencia Laboral</span>
                                    <button type="button" @click="addExperiencia" class="text-primary font-bold text-xs hover:underline">+ Agregar</button>
                                </div>
                                <template x-for="(e, i) in form.experiencias" :key="i">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-5 bg-white border border-slate-100 rounded-2xl shadow-sm relative animate-fade-up">
                                        <button type="button" @click="removeExperiencia(i)" class="absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center"><i data-lucide="x" class="w-3 h-3"></i></button>
                                        <input type="text" x-model="e.cargo" placeholder="Cargo / Rol" class="input-gold py-2 text-sm">
                                        <input type="text" x-model="e.empresa" placeholder="Empresa / Entidad" class="input-gold py-2 text-sm">
                                    </div>
                                </template>
                            </div>

                            <div>
                                <label class="label-gold">Otras Habilidades</label>
                                <input type="text" x-model="form.habilidades" placeholder="Ej: Oratoria, Manejo de Grupos, Redes Sociales..." class="input-gold">
                            </div>
                        </div>
                    </div>

                    <!-- SUBMIT -->
                    <div x-show="form.campana_id" x-cloak class="pt-10 text-center">
                        <button type="submit" 
                            ...
                            <span x-show="!isSubmitting">Finalizar mi Registro</span>
    ...
    <footer class="text-center pt-8 pb-12 text-slate-400 relative z-10">
        ...
        <p class="text-[11px] font-bold">Modulo de Líderes <span class="text-primary tracking-widest px-2 py-1 bg-white rounded-md border border-slate-100 ml-1">v2.6.8 Gold</span></p>
        <div class="mt-4 inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-widest bg-emerald-50 text-emerald-600 px-4 py-1.5 rounded-full border border-emerald-100">
...
function registrationForm() {
    return {
        ...
        selectedLiderName: '',
        rawMunicipio: '',
        allAreas: ['Gestión Territorial', 'Logística', 'Comunicaciones', 'Electoral', 'Jurídico', 'Sistemas IT', 'Poblacional', 'Publicidad'],
        ...
        form: { 
            campana_id: '', nombres: '', apellidos: '', documento: '', telefono: '',
            departamento: '', municipio: '', cod_mpio: '',
            tipo_territorio: '', territorio: '', barrio: '',
            puesto_votacion: '', mesa_votacion: '',
            lider_directo: '', areas_interes: [],
            resumen_profesional: '', formaciones: [], experiencias: [], habilidades: '',
            habeas_data: true
        },
        ...
        async searchLideres() {
            ...
            const data = await this.apiFetch('/api/colaboradores/lideres', { campana_id: this.form.campana_id, search: this.searchQuery });
        ...
        addFormacion() { this.form.formaciones.push({ titulo: '', institucion: '' }); setTimeout(() => lucide.createIcons(), 50); },
        removeFormacion(i) { this.form.formaciones.splice(i, 1); },
        addExperiencia() { this.form.experiencias.push({ cargo: '', empresa: '' }); setTimeout(() => lucide.createIcons(), 50); },
        removeExperiencia(i) { this.form.experiencias.splice(i, 1); },
        ...
        async submitForm() {
            ...
            try {
                const response = await fetch('/registro-lider', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(this.form)
                });
                const data = await response.json();
                if (data.success) { alert('¡Registro exitoso como Líder!'); location.href = '/'; } 
