...
require_once dirname(__DIR__) . '/Library/PHPMailer/Exception.php';
require_once dirname(__DIR__) . '/Library/PHPMailer/PHPMailer.php';
require_once dirname(__DIR__) . '/Library/PHPMailer/SMTP.php';
...
class Email
{
    ...
    public static function send(string $to, string $subject, string $body, string $altBody = ''): bool
    {
        ...
        try {
            ...
            $mail->SMTPDebug = SMTP::DEBUG_SERVER; // Habilitar solo para depuración profunda
            $mail->isSMTP();
            ...
            $errorMsg = "Error al enviar email a {$to}: {$mail->ErrorInfo}";
            Logger::error($errorMsg);
            error_log($errorMsg); // También al log del sistema
            if (APP_DEBUG) {
