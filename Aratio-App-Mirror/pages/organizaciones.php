<?php
/**
 * Módulo Organizaciones v2.0.0 — Admin CRUD con Estructuras Legales
 * Acceso: index.php?page=organizaciones
 */
...
.bloque-header{background:linear-gradient(90deg, <?= COLOR_PRIMARY ?> 0%, <?= COLOR_ACCENT ?> 100%);color:white;padding:6px 12px;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:1px}
...
<div class="space-y-6 pb-20" x-data="jacAdmin()" x-init="init()">
    ...
    <div>
      <h1 class="text-3xl font-black text-primary tracking-tight flex items-center gap-3">
        <div class="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center shadow-lg">
          <i data-lucide="building-2" class="w-6 h-6 text-secondary"></i>
        </div>
        Organizaciones
      </h1>
      <p class="text-gray-500 font-medium mt-1 ml-15 italic">Gestión de Organizaciones — Aratio Pro</p>
    </div>
    <?php if ($campanaActiva): ?>
    <div class="flex items-center gap-3">
      <a href="<?= url('?page=dashboard_organizaciones_publico&campana_id='.$campanaId) ?>" target="_blank"
         class="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-primary rounded-xl font-black text-sm hover:shadow-md transition">
        <i data-lucide="map" class="w-4 h-4"></i> Ver Dashboard
      </a>
      <?php if ($isAdmin): ?>
      <button @click="openJacModal()"
              class="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-xl font-black text-sm hover:bg-secondary hover:text-primary transition shadow-xl">
        <i data-lucide="plus" class="w-5 h-5"></i> Nueva Organización
      </button>
      <?php endif; ?>
    </div>
    <?php endif; ?>
  ...
  <div class="jac-card rounded-3xl overflow-hidden p-6 flex flex-wrap gap-4 items-center">
    <div class="relative flex-1 min-w-[300px]">
        ...
        <input type="text" x-model="filters.search" @input.debounce.400ms="loadData()"
               ...
               class="pl-10 pr-4 py-3 border border-gray-100 rounded-2xl text-sm focus:ring-2 focus:ring-primary w-full bg-gray-50/50">
    </div>
    <select x-model="filters.municipio" @change="loadData()"
            class="px-4 py-3 border border-gray-100 rounded-2xl text-sm bg-gray-50/50 focus:ring-2 focus:ring-primary">
  ...
  <div class="grid grid-cols-1 gap-4">
    <template x-if="loading">
        <div class="py-20 text-center"><i data-lucide="loader-2" class="w-10 h-10 spin mx-auto text-primary"></i></div>
    </template>
        ...
        <div class="jac-card rounded-3xl overflow-hidden group">
            <div class="p-6 transition-colors hover:bg-gray-50/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-5">
                    <div class="w-14 h-14 bg-primary/5 rounded-2xl flex items-center justify-center group-hover:bg-primary transition-all">
                        <i data-lucide="building-2" class="w-6 h-6 text-primary group-hover:text-secondary"></i>
                    </div>
                    <div>
                        <h4 class="text-lg font-black text-primary uppercase tracking-tight" x-text="row.nombre_jac"></h4>
                        <div class="flex items-center gap-2 mt-1">
                            <span class="px-2 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-black rounded" x-text="row.tipo_organizacion || 'Organización'"></span>
                ...
                <div class="grid grid-cols-2 gap-6 text-sm flex-1 max-w-xl">
                    <div>
                        ...
                        <p class="font-bold text-primary" x-text="row.presidente || 'No asignado'"></p>
                    ...
                    <div class="hidden lg:block">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div><p class="font-black text-secondary" x-text="row.votos_comprometidos || 0"></p><p class="text-[8px] uppercase text-gray-400">Votos</p></div>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="expanded === row.id" x-cloak x-transition class="border-t border-gray-50 bg-gray-50/30 p-8 pt-6">
                <div class="flex items-center justify-between mb-6">
                    <h5 class="text-sm font-black text-primary uppercase tracking-widest flex items-center gap-2">
                        <i data-lucide="users" class="w-4 h-4 text-accent"></i>
                        Estructura Organizacional
                    </h5>
                    <div class="flex gap-2">
                      <button @click="openNewPlancha(row)" class="px-4 py-2 bg-primary text-white text-[10px] font-black rounded-lg hover:bg-accent transition">CREAR NUEVA ESTRUCTURA</button>
                    </div>
                </div>
                    ...
                    <template x-for="pl in (planchasCache[row.id] || [])" :key="pl.id">
                        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                            <div class="p-4 bg-gray-50/50 flex items-center justify-between border-b border-gray-50">
                                <div>
                                    <span class="font-black text-primary text-sm uppercase mr-2" x-text="pl.nombre"></span>
                                ...
                                <div class="flex gap-1">
                                    <button @click="editPlancha(pl, row)" class="p-1.5 text-gray-400 hover:text-primary"><i data-lucide="edit-3" class="w-3 h-3"></i></button>
                                        ...
                                        <table class="w-full text-[11px]">
                                            <template x-for="m in (pl.miembros || []).filter(mb => mb.bloque === bloq)" :key="m.id">
                                                <tr class="border-b border-gray-50">
                                                    <td class="px-4 py-2 font-bold text-primary w-1/2" x-text="m.nombre"></td>
  ...
  <div x-show="showJacModal" class="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50" x-cloak>
    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-hidden flex flex-col">
        <div class="p-8 border-b border-gray-50 flex items-center justify-between">
            <div>
                <h3 class="text-2xl font-black text-primary" x-text="isEditingJac ? 'Actualizar Organización' : 'Nueva Organización'"></h3>
        ...
        <form @submit.prevent="saveJac()" class="p-8 space-y-6 overflow-y-auto">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2 space-y-1">
                    ...
                    <input type="text" x-model="jacForm.nombre_jac" required class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary" placeholder="Ej: Club Deportivo X / JAC Barrio Popular">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <select x-model="jacForm.depto" required @change="onDeptoChange()" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary">
                ...
                <div class="space-y-1">
                    ...
                    <select x-model="jacForm.municipio" required @change="onMpioChange()" :disabled="!jacForm.depto" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary disabled:opacity-50">
                        <option value="">Seleccione...</option>
                ...
                <div class="space-y-1">
                    ...
                    <select x-model="jacForm.tipo_territorio" required @change="onTipoChange()" :disabled="!jacForm.municipio" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary disabled:opacity-50">
                        <option value="">Seleccione...</option>
                ...
                <div class="space-y-1">
                    ...
                    <select x-model="jacForm.territorio_valle_nombre" required @change="onSectorChange()" :disabled="!jacForm.tipo_territorio" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary disabled:opacity-50">
                        <option value="">Seleccione...</option>
                ...
                <div class="space-y-1">
                    ...
                    <select x-model="jacForm.sector" required :disabled="!jacForm.territorio_valle_nombre" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary disabled:opacity-50">
                        <option value="">Seleccione...</option>
                ...
                <div class="md:col-span-2 space-y-1 mt-2 p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
                    <label class="text-[10px] font-black text-primary uppercase tracking-widest pl-1">Clasificación Específica *</label>
                    <select x-model="jacForm.tipo_organizacion" required class="w-full px-5 py-4 bg-white border-none shadow-sm rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary text-primary">
            ...
            <div class="flex justify-end gap-3 mt-4">
                ...
                <button type="submit" class="px-8 py-3 bg-primary text-white rounded-2xl text-sm font-black shadow-xl hover:bg-secondary transition">GUARDAR</button>
            </div>
        </form>
    </div>
  </div>
  ...
  <div x-show="showPlanchaModal" class="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center p-4 z-[60]" x-cloak>
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col">
        <div class="p-8 bg-primary text-white flex items-center justify-between">
            <div>
                <h3 class="text-2xl font-black italic tracking-tight" x-text="isEditingPlancha ? 'Editar Estructura Organizacional' : 'Nueva Estructura Organizacional'"></h3>
                <p class="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] mt-1" x-text="'Organizacion: ' + (currentJac ? currentJac.nombre_jac : '')"></p>
            </div>
        ...
        <div class="p-8 flex-1 overflow-y-auto space-y-8 bg-gray-50/50">
            <!-- Nombre de Plancha -->
            <div class="max-w-md">
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Título de la Estructura *</label>
                <input type="text" x-model="planchaForm.nombre" class="w-full px-6 py-4 bg-white border-2 border-gray-100 rounded-2xl text-lg font-black text-primary outline-none focus:border-secondary transition-all" placeholder="Ej: Plancha 1 - Mi Barrio Crece">
            </div>
            ...
            <div class="grid grid-cols-1 gap-12">
                <template x-for="bloq in bloquesLegales" :key="bloq">
                    <div class="space-y-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 bg-accent rounded-lg flex items-center justify-center text-white"><i data-lucide="shield" class="w-4 h-4"></i></div>
                            <h4 class="text-sm font-black text-primary uppercase tracking-wider" x-text="bloq"></h4>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <template x-for="(m, idx) in planchaForm.miembros.filter(item => item.bloque === bloq)" :key="idx">
                                <div class="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3 relative group">
                                    ...
                                    <input type="text" x-model="m.nombre" class="w-full bg-transparent border-b border-gray-100 py-1 font-black text-primary text-sm focus:border-accent outline-none" placeholder="Nombre Completo">
                                    <div class="flex gap-2">
                            ...
                            <button @click="addMiembro(bloq)" class="p-5 border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center gap-2 hover:border-accent hover:bg-accent/5 transition group">
                                <i data-lucide="plus" class="w-6 h-6 text-gray-300 group-hover:text-accent"></i>
                                <span class="text-[9px] font-black text-gray-400 group-hover:text-accent uppercase">Añadir a <span x-text="bloq"></span></span>
                            </button>
                        </div>
                    </div>
                </template>
        ...
        <div class="p-8 border-t border-gray-100 flex items-center justify-between bg-white">
            <div class="flex items-center gap-6">
                 <div><p class="text-lg font-black text-primary" x-text="planchaForm.miembros.length"></p><p class="text-[8px] font-black text-gray-400 uppercase tracking-widest">Postulados</p></div>
                 <div class="w-px h-10 bg-gray-100"></div>
                 <div><p class="text-lg font-black text-accent" x-text="Math.round((planchaForm.miembros.filter(m=>m.genero==='F').length / (planchaForm.miembros.length||1)) * 100) + '%'"></p><p class="text-[8px] font-black text-gray-400 uppercase tracking-widest">Cuota Género Fem</p></div>
            </div>
            <div class="flex gap-4">
                ...
                <button @click="savePlancha()" class="px-12 py-4 bg-primary text-white rounded-2xl text-sm font-black shadow-2xl hover:bg-accent transition">GUARDAR & CERRAR</button>
            </div>
        </div>
    </div>
  </div>
...
function jacAdmin() {
  return {
    ...
    async loadData() {
      ...
      const r = await fetch(`${this.API_BASE}organizaciones.php?${params}`);
    ...
    async loadMunicipios() {
      const r = await fetch(`${this.API_BASE}organizaciones.php?action=municipios&campana_id=${this.campanaId}`);
    ...
    async loadPlanchas(jacId) {
      ...
      const r = await fetch(`${this.API_BASE}organizaciones.php?action=planchas&jac_id=${jacId}`);
    ...
    async loadGeoDeptos() {
        const r = await fetch(`${this.API_BASE}territorios.php?accion=departamentos`);
    ...
    async onDeptoChange() {
        ...
        const r = await fetch(`${this.API_BASE}territorios.php?accion=municipios&departamento=${encodeURIComponent(this.jacForm.depto)}`);
    ...
    async onMpioChange() {
        ...
        const r = await fetch(`${this.API_BASE}territorios.php?accion=tipos_territorio&departamento=${encodeURIComponent(this.jacForm.depto)}&municipio=${encodeURIComponent(this.jacForm.municipio)}`);
    ...
    async onTipoChange() {
        ...
        const r = await fetch(`${this.API_BASE}territorios.php?accion=territorios&departamento=${encodeURIComponent(this.jacForm.depto)}&municipio=${encodeURIComponent(this.jacForm.municipio)}&tipo_territorio=${encodeURIComponent(this.jacForm.tipo_territorio)}`);
        const d = await r.json(); if(d.success) this.geos.sectores = d.data;
    },
    async onSectorChange() {
        this.geos.barrios=[];
        this.jacForm.sector='';
        const r = await fetch(`${this.API_BASE}territorios.php?accion=barrios&departamento=${encodeURIComponent(this.jacForm.depto)}&municipio=${encodeURIComponent(this.jacForm.municipio)}&tipo_territorio=${encodeURIComponent(this.jacForm.tipo_territorio)}&territorio=${encodeURIComponent(this.jacForm.territorio_valle_nombre)}`);
    ...
    async editJac(row) {
        ...
        const r1 = await fetch(`${this.API_BASE}territorios.php?accion=municipios&departamento=${encodeURIComponent(currentData.depto)}`);
        const d1 = await r1.json(); if(d1.success) this.geos.mpios = [...new Set(d1.data.map(m => (typeof m === 'object' ? m.municipio : m)))];
        
        const r2 = await fetch(`${this.API_BASE}territorios.php?accion=tipos_territorio&departamento=${encodeURIComponent(currentData.depto)}&municipio=${encodeURIComponent(currentData.municipio)}`);
        const d2 = await r2.json(); if(d2.success) this.geos.tipos = d2.data;
        
        const r3 = await fetch(`${this.API_BASE}territorios.php?accion=territorios&departamento=${encodeURIComponent(currentData.depto)}&municipio=${encodeURIComponent(currentData.municipio)}&tipo_territorio=${encodeURIComponent(currentData.tipo_territorio)}`);
        const d3 = await r3.json(); if(d3.success) this.geos.sectores = d3.data;
        
        const r4 = await fetch(`${this.API_BASE}territorios.php?accion=barrios&departamento=${encodeURIComponent(currentData.depto)}&municipio=${encodeURIComponent(currentData.municipio)}&tipo_territorio=${encodeURIComponent(currentData.tipo_territorio)}&territorio=${encodeURIComponent(currentData.territorio_valle_nombre)}`);
    ...
    async saveJac() {
        ...
        const r = await fetch(`${this.API_BASE}organizaciones.php`, { method, headers:{'Content-Type':'application/json'}, body:JSON.stringify(this.jacForm) });
        const d = await r.json();
        if(d.success) { this.showJacModal = false; this.loadData(); }
    },
    async deleteJac(id, name) {
        if(!confirm(`¿Eliminar Organización ${name}?`)) return;
        await fetch(`${this.API_BASE}organizaciones.php?id=${id}`, { method:'DELETE' }); this.loadData();
    },
    ...
    async savePlancha() {
        ...
        const r = await fetch(`${this.API_BASE}organizaciones.php?action=plancha`, { method, headers:{'Content-Type':'application/json'}, body:JSON.stringify(this.planchaForm) });
        const d = await r.json();
        if(d.success) { 
    ...
    async activarPlancha(planchaId, jacId) {
        await fetch(`${this.API_BASE}organizaciones.php?action=activar_plancha&id=${planchaId}`, { method:'PUT' });
        this.loadPlanchas(jacId); this.loadData();
    },
    async deletePlancha(pid, name, jid) {
        if(!confirm(`¿Eliminar plancha ${name}?`)) return;
        await fetch(`${this.API_BASE}organizaciones.php?action=plancha&id=${pid}`, { method:'DELETE' });
