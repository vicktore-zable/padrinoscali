                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select x-model="municipio_raw" @change="selectMunicipio()" required
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                            ...
                            <div>
                                ...
                                <select x-model="form.barrio" 
                                        @change="const b = listas.barrios.find(x => x.barrio === $event.target.value); form.territorio_id = b ? b.id : null"
                                        required
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                                        :disabled="!form.tipo_territorio">
                                    <option value="">Seleccionar barrio...</option>
                                    <template x-for="b in listas.barrios" :key="b.id">
                                        <option :value="b.barrio" x-text="b.barrio"></option>
                                    </template>
                                </select>
                                <input type="hidden" x-model="form.territorio_id">
                            </div>
                        </div>
                    </div>
    ...
    function registroData() {
        return {
            ...
            async cargarBarrios() {
                ...
                try {
                    const response = await fetch(`/api/territorios.php?accion=barrios_v2&departamento=${encodeURIComponent(this.form.departamento)}&municipio=${encodeURIComponent(this.form.municipio)}&tipo_territorio=${encodeURIComponent(this.form.tipo_territorio)}`);
                    const result = await response.json();
                    if (result.success) {
            ...
            async submitForm() {
                if (!this.form.hasSignature) {
                    this.message = 'Por favor, agregue su firma digital';
                    this.messageType = 'error';
                    return;
                }

                this.form.firma = this.canvas.toDataURL('image/png');
                this.loading = true;
                this.message = '';
